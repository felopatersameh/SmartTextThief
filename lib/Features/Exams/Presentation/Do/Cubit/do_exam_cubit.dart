...
import '../../../shared/Models/Results/option_model.dart';
...
class SolveExamCubit extends Cubit<SolveExamState> {
  ...
  List<QuestionModel> _shuffleExam(
    ...
    return shuffledQuestions.map((question) {
      if (question.type == ExamQuestionType.multipleChoice.value &&
          (question.options?.length ?? 0) > 1) {
        final shuffledOptions = List<OptionModel>.from((question.options ?? []))
          ..shuffle(random);
  ...
  List<QuestionModel> _mapQuestions(List<QuestionModel> questions) {
    return questions
        .map(
