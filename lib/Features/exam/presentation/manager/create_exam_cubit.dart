...
import 'package:smart_text_thief/Features/exam/data/models/information_file_model.dart';
import 'package:smart_text_thief/Features/exam/data/repositories/create_exam_repository.dart';

import '../../../../Config/Routes/Routes/view_exam_route.dart';
...
class CreateExamCubit extends Cubit<CreateExamState> {
  ...
  Future<void> submitExam(BuildContext context) async {
    ...
    final totalQuestions = (int.tryParse(currentState.numMultipleChoice??'0') ?? 0) +
        (int.tryParse(currentState.numTrueFalse??'0') ?? 0) +
        (int.tryParse(currentState.numQA??'0') ?? 0);
  ...
  Future<void> created(BuildContext context) async {
    final numChose = state.numMultipleChoice??'0';
    final numTF = state.numTrueFalse??'0';
    final numQA = state.numQA??'0';
    final profileApiKey = '';
